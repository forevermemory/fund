...
def tt_do_search_zhishu_detail(param:dict):
    '''搜素指数基金详情'''
...
from sql import Bond,sql_session
from tool import _tt_do_search_fund_item
...
def tt_do_get_bond_fund_detail_nh_hc(out_dir:str):
    ...
    # cache
    fonds = sql_session.query(Bond).all()
    fkv = {}
    for f in fonds:
        fkv[f.code] = f
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            _my_print(f"处理err:{length}-{index}-{f.name}")
            continue
        f: Bond
        ids.append(f.id)

        if f.update_flag == up_flag:
            continue
